# -*- coding: utf-8 -*-
"""
Cross-correlation function (CCF) toolkit for radial-velocity measurements.

This module implements the CCF machinery used by the GHOST Pipeline to
measure precise radial velocities: reading and weighting stellar line masks,
computing per-order and mean CCFs, fitting the CCF with a Gaussian model,
and deriving diagnostic quantities (FWHM and bisector span). The main entry
points are:

* ``set_ccf_params``: builds the CCF parameter dictionary for a given mask.
* ``apply_weights_to_ccf_mask``: projects the mask onto a spectrum and
  computes the line weights.
* ``run_ccf_eder``: computes the CCF of a single spectrum and measures RV.
* ``ccf_analysis``: template-matching analysis of a CCF time series.
* ``ccf_fwhm_and_biss``: FWHM and bisector time series from the CCFs.
* ``bisector``: CCF bisector measurement.

Adapted from the CCF codes of the APERO data reduction software for
SPIRou (original author: Neil Cook, 2020-03-27); adapted by
Eder Martioli starting on 2020-04-28.
...
def set_ccf_params(maskfile, telluric_masks=[], science_channel=True) :
    """Build the dictionary of CCF parameters for a given line mask.

    Parameters
    ----------
    maskfile : str
        Path to the CCF line mask file (.mas).
    telluric_masks : list of str, optional
        Paths to telluric line masks, if any.
    science_channel : bool, optional
        If True, set parameters for the science channel; if False, for the
        calibration (drift) channel.

    Returns
    -------
    dict
        Dictionary of CCF parameters (mask info, CCF width, step, target
        RV, fit type, etc.) used by the CCF routines in this module.
    """
    ...
    if science_channel :
        ...
        loc["CCF_STEP"] = 0.01                     # CCF_DEFAULT_STEP (or user input)
...
def apply_weights_to_ccf_mask(ccf_params, wl, flux, fluxerr, weight, median=True, remove_lines_with_nans=True, source_rv=0., verbose=False, plot=False) :
    """Project the CCF mask onto a spectrum and compute the line weights.

    The mask lines are trimmed to the spectral range, shifted by the
    source RV, and each line receives a weight given by the product of the
    original mask weight and the statistical weight of the spectrum at the
    line position.

    Parameters
    ----------
    ccf_params : dict
        CCF parameters returned by :func:`set_ccf_params`.
    wl, flux, fluxerr : array-like
        Spectrum wavelength (nm), flux and flux errors.
    weight : array-like
        Statistical weights of the spectrum (see
        :func:`ghostpipe.reduc_lib.calculate_weights`).
    median : bool, optional
        If True, use the median (instead of the sum) of the weights within
        each line region.
    remove_lines_with_nans : bool, optional
        If True, discard mask lines with non-finite weights.
    source_rv : float, optional
        Source radial velocity (km/s) used to shift the mask.
    verbose, plot : bool, optional
        Print progress information / plot the weighted mask.

    Returns
    -------
    dict
        Mask container with the line centers, widths, and final weights.
    """
...
def run_ccf_eder(ccf_params, wave, fluxes, header, ccfmask, rv_drifts={}, filename="", berv=0., targetrv=0.0, normalize_ccfs=True, fit_type=0, output="", plot=False, verbose=False) :
    """Compute the CCF of a spectrum and measure its radial velocity.

    This is an adapted version of the APERO/SPIRou CCF routine that takes
    a reduced (s1d or order-by-order) spectrum as input, computes the CCF
    with the weighted line mask, fits the mean CCF and stores the results
    (RV, FWHM, contrast, drift) in the output properties dictionary. If
    ``output`` is given, a CCF table FITS product is saved.

    Parameters
    ----------
    ccf_params : dict
        CCF parameters returned by :func:`set_ccf_params`.
    wave, fluxes : array-like
        Wavelengths (nm) and fluxes; either 1D (s1d) or 2D (per order).
    header : astropy.io.fits.Header
        Header of the input spectrum (must contain the BERV, unless given
        through the ``berv`` argument).
    ccfmask : dict
        Weighted mask returned by :func:`apply_weights_to_ccf_mask`.
    rv_drifts : dict, optional
        Dictionary with instrumental RV drift keywords; zero drift is
        assumed if empty.
    filename : str, optional
        Name of the input spectrum file (bookkeeping only).
    berv : float, optional
        Barycentric Earth radial velocity (km/s), used if not in header.
    targetrv : float, optional
        Initial guess for the source RV (km/s).
    normalize_ccfs : bool, optional
        If True, normalize the CCFs by the continuum.
    fit_type : int, optional
        CCF fit type (0: Gaussian for absorption lines).
    output : str, optional
        Output CCF FITS table path; if empty, no file is saved.
    plot, verbose : bool, optional
        Plot the CCFs / print progress information.

    Returns
    -------
    dict
        Properties dictionary with the CCF data (``RV_CCF``,
        ``MEAN_CCF``), the updated ``header`` (with ``RV_OBJ``,
        ``CCFMFWHM``, ``RV_DRIFT``, etc.), and, when ``output`` is given,
        the ``file_path`` of the saved product.
    """
...
def save_ccfs_to_fits(template_ccf, bjds, filename, header=None) :
    """Save a CCF time series to a FITS product.

    The output file contains the image extensions ``RV`` (velocity grid),
    ``MEANCCF`` (template CCF), ``CCFS`` (individual CCFs),
    ``RESIDUAL_CCFS`` (CCF residuals) and ``TIMES`` (BJDs).

    Parameters
    ----------
    template_ccf : dict
        CCF template container returned by :func:`ccf_analysis`.
    bjds : array-like
        Times of observation (BJD), one per exposure.
    filename : str
    ...
    hdu_resccfs = fits.ImageHDU(data=residuals, name="RESIDUAL_CCFS")
    hdu_times = fits.ImageHDU(data=bjds, name="TIMES")

    mef_hdu = fits.HDUList([primary_hdu, hdu_rv, hdu_mccf, hdu_ccfs, hdu_resccfs, hdu_times])
...
def ccf_analysis(rv, ccf_data, rvs, nsig_clip=0, velocity_window=10., maxiter=20, minimprov=1e-5, plot=False, verbose=False) :
    """Template-matching analysis of a CCF time series.

    Builds a template (mean) CCF and iteratively measures the relative RV
    shift of each individual CCF with respect to the template, until the
    RV corrections converge.

    Parameters
    ----------
    rv : array-like
        Velocity grid of the CCFs (km/s).
    ccf_data : list of array-like
        Individual CCFs, one per exposure.
    rvs : array-like
        Initial RV estimates (km/s), e.g. from the Gaussian CCF fits.
    nsig_clip : float, optional
        Sigma-clip threshold applied in the template calculation.
    velocity_window : float, optional
        Half-width (km/s) of the window around the line center used in
        the template matching.
    maxiter : int, optional
        Maximum number of iterations.
    minimprov : float, optional
        Minimum relative improvement to continue iterating.
    plot, verbose : bool, optional
        Plot the CCF template products / print progress information.

    Returns
    -------
    tuple
        ``(rvs, rverrs, template_ccf)`` with the measured RVs and
        uncertainties (km/s), one per exposure, and the CCF template
        container.
    """
...
def ccf_fwhm_and_biss(template_ccf, velocity_window=0, use_tight_window=False, plot=False, verbose=False) :
    """Measure the FWHM and bisector span time series from the CCFs.
    ...
    template_ccf : dict
        CCF template container returned by :func:`ccf_analysis`.
    velocity_window : float, optional
        Half-width (km/s) of the window around the line center; if 0, the
        full velocity range is used.
    use_tight_window : bool, optional
        If True, restrict measurements to the tight window.
    plot, verbose : bool, optional
        Plot the fits / print progress information.

    Returns
    -------
    tuple
        ``(fwhm, fwhmerr, bis, biserr)`` arrays (km/s), one entry per
        exposure.
    """
