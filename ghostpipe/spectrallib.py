# -*- coding: utf-8 -*-
"""
Spectral quantities and stellar activity indicators.

This module contains routines to measure quantities from 1D spectra,
with an emphasis on chromospheric activity indices:

* ``sindex`` / ``sindex_montecarlo``: Ca II H&K S-index measurement, with
  Monte Carlo error estimation.
* ``activity_index`` / ``activity_index_montecarlo`` /
  ``activity_index_timeseries``: generic line activity indices
  (e.g. H-alpha, Ca I) and their time series.
* ``calibrate_sindex_to_MW``: calibration of the instrumental S-index to
  the Mount Wilson system, and conversion to log R'HK using several
  calibrations from the literature.
* ``extract_spectral_feature``: extraction and normalization of a spectral
  region around a feature of interest.
* ``spectral_errors_from_residuals``: empirical flux error estimation from
  the dispersion of template residuals.
* ``save_time_series`` / ``save_rv_time_series``: writers for rdb time
  series products.
* ``write_spectra_times_series_to_fits`` / ``read_template_product``:
  FITS I/O for spectral time series (template) products.
...
def spectral_errors_from_residuals(wl, residuals, max_delta_wl=0.2, halfbinsize=15, min_points_per_bin=3, use_mad=True) :
    ...
    if use_mad :
        errors = stats.median_abs_deviation(arrays, axis=0, nan_policy='omit')
    else :
        errors = stats.median_abs_deviation(arrays, axis=0, nan_policy='omit')
    ...
    for i in range(npoints) :
        ...
        if len(residchunk[keep]) >= min_points_per_bin :
            if use_mad :
                errors[i] = stats.median_abs_deviation(residchunk[keep],nan_policy='omit')
            else :
...
def activity_index_montecarlo(wl, flux, fluxerr, wl_lines=[656.28], delta_wl_lines=[0.15], wl_conts=[655.087,658.031], delta_wl_conts=[1.075,0.875], nsamples=1000, line_label=r"H$\alpha$", verbose=False, plot=False) :
