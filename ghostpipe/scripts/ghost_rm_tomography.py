# -*- coding: utf-8 -*-
"""
Doppler tomography of the Rossiter-McLaughlin effect from GHOST CCFs.

Given a CCF time series FITS product (as produced by
``ghost_ccf_pipeline``) obtained during a planetary transit, this tool
computes and plots the time series of CCF residuals to reveal the Doppler
shadow of the planet (Rossiter-McLaughlin effect), along with the transit
model of the anomalous RV signal.

Usage example
-------------
::

    ghost_rm_tomography --input=target_ghost_red_ccfs.fits -p
...
def main():
    """Entry point: parse command-line options and run the RM tomography analysis."""
    parser = OptionParser()
    ...
    # Expected structure of the input CCF FITS product (from ghost_ccf_pipeline):
    #   1  RV             ImageHDU  (nvels,)        velocity grid [km/s]
    #   2  MEANCCF        ImageHDU  (nvels,)        template (mean) CCF
    #   3  CCFS           ImageHDU  (nvels, nspec)  individual CCFs
    #   4  RESIDUAL_CCFS  ImageHDU  (nvels, nspec)  CCF residuals (CCF - template)
    #   5  TIMES          ImageHDU  (nspec,)        times of observation [BJD]
