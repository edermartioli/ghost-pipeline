# -*- coding: utf-8 -*-
"""
Spectral reduction utilities for time series of GHOST s1d spectra.

This module (originally developed for SPIRou data and adapted to GHOST)
contains routines to load, resample, align, combine, and normalize time
series of 1D spectra, and to build high signal-to-noise template spectra.
The main entry points are:

* ``load_array_of_ghost_spectra``: loads a list of s1d FITS spectra,
  optionally applying the barycentric correction and source RV shifts.
* ``get_spectral_data`` / ``set_common_wl_grid`` /
  ``resample_and_align_spectra``: put all spectra of a time series onto a
  common wavelength grid.
* ``reduce_spectra`` / ``calculate_template`` / ``sigma_clip``: iterative
  template construction with sigma-clipping of outliers.
* ``fit_continuum`` / ``normalize_spectra``: continuum fitting and
  normalization.
* ``calculate_weights``: statistical weights for the CCF mask lines.
* ``reduce_timeseries_of_spectra``: high-level wrapper that chains the
  steps above and prepares the inputs for the CCF analysis.
...
try:
    ...
    from numpy import RankWarning as _RankWarning
...
def load_array_of_ghost_spectra(inputdata, rvfile="", apply_berv=True, silent=True, obslog="", plot_spectra=False, plot=False, verbose=False) :
    """Load a time series of GHOST s1d spectra into a container dict.

    Each input spectrum is loaded with
    :func:`ghostpipe.ghostlib.load_spectrum`; wavelengths are stored both
    in the observed frame and shifted to the stellar (source) frame and to
    the barycentric rest frame, using the BERV in the headers and,
    optionally, an input RV time series.

    Parameters
    ----------
    inputdata : list of str
        List of s1d FITS file paths, one per exposure.
    rvfile : str, optional
        Path to an rdb file with the source RVs (km/s) for each exposure;
        if empty, a null RV is assumed for all exposures.
    apply_berv : bool, optional
        If True, apply the barycentric velocity correction.
    silent : bool, optional
        If True, suppress warnings while reading the RV file.
    obslog : str, optional
        Path of an output observation log file; if empty, no log is saved.
    plot_spectra, plot : bool, optional
        If True, plot the spectra / RV time series.
    ...
    -------
    dict
        Container with keys ``spectra`` (list of spectrum dicts),
        ``nspectra``, and bookkeeping arrays (BJDs, airmasses, RVs, etc.).
    """
    ...
    wl1,wl2=300,1100 # all

    #wl1,wl2=467,470
    #wl1,wl2=455,475 # HeII
    #wl1,wl2=480,495 # H-beta
    #wl1,wl2=500,530 # reddest part of blue channel
    #wl1,wl2=870,882 # Pa12
    #wl1,wl2=570.5,571.9 # N2??
    
    wl_out = np.linspace(wl1+0.1, wl2-0.1, 1000)
    objectname = ""
    for i in range(len(inputdata)) :
        
        spectrum = ghostlib.load_spectrum(inputdata[i], minwl=360, minflux=1)
        ...
        #hdr = ghostlib.set_berv_bjd(hdr, use_jpl_ephemeris=False)
        
        spectrum['OBJECT'] = hdr['OBJECT']
        spectrum['DATE'] = hdr['ISOTDATE']
                
        spectrum['BJD'] = hdr["BJD"]
        ...
        allsnr = spectrum['flux'] / spectrum['fluxerr']
        pcen=np.percentile(allsnr,99.,method='nearest')
        i_near = np.abs(allsnr-pcen).argmin()
        spectrum['SNR'] = allsnr[i_near] # need calculate
        spectrum["header"]['SNR'] = allsnr[i_near]
        
        keep = (spectrum["wl"]>wl1) & (spectrum["wl"]<wl2)
        
        y = spectrum['flux'][keep]
        mean = np.nanmean(y)
        sigma = np.nanstd(y)
        
        #if spectrum['SNR'] < 100 or spectrum['SNR'] > 1500 or sigma > 500:
        #    continue
        #if spectrum['SNR'] < 50 or spectrum['SNR'] > 700 :
        #    continue
        #print("{} EXPTIME={} SNR={:.1f}  Flux: {:.3f} +- {:.3f}".format(spectrum['FILENAME'], spectrum['EXPTIME'], spectrum['SNR'], mean, sigma))
        ...
        if verbose :
            print("Spectrum ({0}/{1}): {2} OBJ={3} BJD={4:.6f} SNR@{5:.1f}nm={6:.1f} EXPTIME={7:.0f}s BERV={8:.3f} km/s".format(i+1,len(inputdata),os.path.basename(inputdata[i]),spectrum['OBJECT'],spectrum['BJD'],spectrum['wl'][i_near],spectrum['SNR'],spectrum['EXPTIME'],spectrum['BERV']))

        if apply_berv :
            #vel_shift = (spectrum['RV'] + spectrum['BERV']) / (1. + spectrum['BERV'] / speed_of_light_in_kps)
        ...
        if plot_spectra :
            ...
            keep = (wl>wl1) & (wl<wl2)
            #keep = wl>360
            #plt.plot(wl[keep],spectrum['flux'][keep]/np.nanmedian(spectrum['flux'][keep]))
            color = [i/len(inputdata),1-i/len(inputdata),1-i/len(inputdata)]
            
            nflux = spectrum['flux'][keep] / np.nanmedian(spectrum['flux'][keep])
            
            f = interp1d(wl_stellar_frame[keep], nflux, bounds_error=False, fill_value='extrapolate', kind='cubic')
            resamp_nflux = f(wl_out)
            
            plt.text(wl_stellar_frame[keep][-1] + 0.2,1.0-0.05*j, "{}".format(hdr['ISOTDATE']),fontsize=8)
            plt.plot(wl_stellar_frame[keep],-0.05*j+nflux,'-',color=color,alpha=0.75,label="{}".format(spectrum['DATE']))
            #plt.plot(wl_stellar_frame[keep],np.gradient(spectrum['flux'][keep]),'.')

            loc_flux.append(resamp_nflux)
    ...
    if plot_spectra:
        ...
        loc_flux = np.array(loc_flux,dtype=float)
        
        #template = calculate_template(loc_flux, wl=wl_out, fit=True, median=True, subtract=True, sub_flux_base=1.0, verbose=False, plot=False)
        #fluxes = template["flux_arr_sub"]

        y = np.array(bjd)
        x_lab = r"Wavelength [nm]"
        y_lab = r"Time"
        z_lab = r"flux"
        
        LAB = [x_lab,y_lab,z_lab]
        #plot_2d(wl_out, y, loc_flux, LIM=None, LAB=LAB, z_lim= [0.7,1.3], title="", pfilename="", cmap="gist_heat")
        plot_2d(wl_out, y, loc_flux, LIM=None, LAB=LAB, z_lim= [0.1,1.9], title="", pfilename="", cmap="gist_heat")
    ...
    if plot :
        ...
        axs[0].set_ylabel('SNR@{:.0f}nm'.format(spectrum['wl'][i_near]))
        axs[0].legend()
...
def get_spectral_data(array_of_spectra, ref_index=0, verbose=False) :
    """Organize an array of spectra into vectors for time-series analysis.

    Parameters
    ----------
    array_of_spectra : dict
        Container returned by :func:`load_array_of_ghost_spectra`.
    ref_index : int, optional
        Index of the exposure used as reference.
    verbose : bool, optional
        If True, print progress information.
    ...
    -------
    dict
        Container with per-exposure arrays of wavelengths (observed,
        source-frame and barycentric-frame), fluxes, flux errors, and
        global quantities (``wlmin``, ``wlmax``, BJDs, etc.).
    """
    if verbose :
        print("Loading data")
...
def set_common_wl_grid(spectra, vel_sampling=0.5, verbose=False) :
    """Define a common wavelength grid with constant velocity sampling.

    Parameters
    ----------
    spectra : dict
        Container returned by :func:`get_spectral_data`; must contain the
        keys ``wlmin`` and ``wlmax``.
    vel_sampling : float, optional
        Velocity sampling of the output grid (km/s).
    verbose : bool, optional
        If True, print progress information.
    ...
    -------
    dict
        The input container updated with the keys ``common_wl`` and
        ``vel_sampling``.
    """
...
def resample_and_align_spectra(spectra, interp_kind='cubic', plot=False, verbose=False) :
    """Interpolate all spectra of a time series onto the common grid.

    Spectra are resampled in the observed, source and barycentric frames
    onto the ``common_wl`` grid defined by :func:`set_common_wl_grid`.

    Parameters
    ----------
    spectra : dict
        Container returned by :func:`get_spectral_data`, after calling
        :func:`set_common_wl_grid`.
    interp_kind : str, optional
        Interpolation kind passed to scipy.interpolate.interp1d.
    plot : bool, optional
        If True, plot the resampled spectra.
    verbose : bool, optional
        If True, print progress information.
    ...
    -------
    dict
        The input container updated with the aligned flux arrays
        (``aligned_waves``, ``sf_fluxes``, ``rest_fluxes``, etc.).
    """
...
def reduce_spectra(spectra, nsig_clip=0.0, combine_by_median=False, combine_final_by_median=False, subtract=True, fluxkey="fluxes", fluxerrkey="fluxerrs", wavekey="wl", update_spectra=False, plot=False, verbose=False) :
    """Build a template and reduce a time series of spectra.

    A template spectrum is calculated for the time series (see
    :func:`calculate_template`), applying an iterative sigma-clipping of
    outliers when ``nsig_clip > 0``.

    Parameters
    ----------
    spectra : dict
        Container of aligned spectra (see
        :func:`resample_and_align_spectra`).
    nsig_clip : float, optional
        Sigma-clip threshold; 0 disables clipping.
    combine_by_median, combine_final_by_median : bool, optional
        Use the median (instead of the mean) to combine spectra in the
        intermediate / final template calculation.
    subtract : bool, optional
        If True, residuals are computed by subtraction; if False, by
        division.
    fluxkey, fluxerrkey, wavekey : str, optional
        Keys of the flux, flux error and wavelength arrays to use.
    update_spectra : bool, optional
        If True, update the flux arrays in the container with the
        sigma-clipped values.
    plot : bool, optional
        If True, plot the template products.
    verbose : bool, optional
        If True, print progress information.
    ...
    -------
    tuple
        ``(spectra, template)`` where ``template`` is the template
        dictionary (see :func:`calculate_template`).
    """
...
def plot_template_products(template, pfilename="") :
    ...
    plt.plot(template["wl"],-sig_clip * template["fluxerr_model"],"-", color="k", lw=0.8, label=r"{0:.0f}$\sigma$ fit model".format(sig_clip))
...
def plot_template_products_with_CCF_mask(template, ccfmask, source_rv=0, wl0=0, wlf=1e10, pfilename="") :
    ...
    for i in range(len(template["flux_arr"])) :
        ...
        if i == len(template["flux_arr"]) - 1 :
            plt.plot(wl, flux,"-", color='#ff7f0e', lw=0.6, alpha=0.6, label="GHOST data", zorder=1)
...
def reduce_timeseries_of_spectra(inputdata, ccf_mask="", object_name="", stellar_spectrum_file="", source_rv=0., max_gap_size=1.0, min_window_size=150., align_spectra=True, vel_sampling=0.5, nsig_clip = 3.0, ccf_width=150, fwhm=8.0, output_template="", fluxkey="rest_fluxes", fluxerrkey="rest_fluxerrs", apply_berv=True, normalize=False, cont_function='spline3', polyn_order=40, save_ccf=False, save_spectra=False, normalize_ccfs=True, plot_ccf=False, plot_details=False, plot=False, verbose=False) :
    ...
    array_of_spectra = load_array_of_ghost_spectra(inputdata, apply_berv=apply_berv, plot=False, verbose=verbose)
    ...
    for i in range(spectra['nspectra']) :
        ...
        if save_spectra :
            ...
            ghostlib.write_spectrum_to_fits(waves_sf, fluxes, template["fluxerr"], output_spectrum_file, header=header)
