# -*- coding: utf-8 -*-
"""
CCF analysis of a set of IAG Solar Atlas spectra.

This tool computes CCFs for a set of solar spectra from the IAG Solar Flux
Atlas (one spectrum per heliocentric angle mu), using the same CCF
machinery as the GHOST pipeline, and saves the resulting CCF time series
to a FITS product.
...
::
    ...
    sun_ccf_pipeline --ccf_mask=masks/G2_nm.mas \
        --input="solarspectrum_mu*.fits" \
        --source_rv=83.74638 --output_ccfs_file="sun_ccfs.fits" -pv
...
def main():
    """Entry point: parse command-line options and run the solar CCF analysis."""
    parser = OptionParser()
